...
- **For what purpose was the dataset created?**
  To support research into corporate bankruptcy prediction using financial statement data. It enables development and benchmarking of ML models for predicting company insolvency, applicable to wholesale credit risk management in financial institutions.

- **Who created the dataset?**
  Maciej Zieba, Sebastian K. Tomczak, and Jakub M. Tomczak at Wroclaw University of Technology, Poland. Full citation: Zieba, M., Tomczak, S.K., Tomczak, J.M. (2016). *Ensemble Boosted Trees with Synthetic Features Generation in Application to Bankruptcy Prediction*. Expert Systems with Applications, 58, 93–101.
...
- **Which cohort file is used in this project?**
  The UCI repository provides five separate annual cohort files for 1-year through 5-year ahead prediction horizons. This project uses the **Year 3 cohort file only** (3-year ahead prediction). This file contains 10,503 observations and is available at `data/polish_bankruptcy.csv`.
...
- **What are the features?**
  ...
  An ablation backtest across 8 feature configurations confirms that 7 of the 14 features account for 99.9% of discriminatory power (LR AUC = 0.9980 vs 0.9989 full). See `results/Feature_Weight_Backtest.xlsx`.
...
- **Is there missing data?**
  ...
  MNAR indicators for leverage (MNAR_X2) and retained earnings (MNAR_X6) are particularly informative: firms suppressing reporting of these ratios are disproportionately distressed.
...
- **Over what time frame?**
  Specific calendar years are not disclosed in the UCI metadata. The 1–5 year forecasting structure implies the underlying data spans at least a 6-year period. The economic cycle covered is unknown, limiting stress-scenario generalisability. It is not known whether the dataset spans the 2008–2009 global financial crisis, which would materially affect representativeness.
...
| MNAR indicators | Binary flags for features > 5% missing in training | Preserves the structural signal of non-reporting, itself a distress indicator |
...
| Class imbalance | class_weight='balanced' (LR); scale_pos_weight ~186 (XGBoost) | Native loss-function correction; avoids synthetic data in training set |
...
Both Logistic Regression and XGBoost were evaluated on this dataset across four independent test dimensions. LR was selected as the primary model on empirical grounds: it produces fewer missed defaults at all industry-standard cost ratios (FN=4 vs 30 at CR=2:1) and superior OOT proper scoring rule performance.

The Spearman rank correlation between LR coefficients and XGBoost SHAP importances is 0.93 (p < 0.0001), confirming near-linear separability of the feature space. This is a dataset property — models applied to this data will find that linear methods match or outperform non-linear ensembles. This finding should be validated on any new dataset before deploying either model.
...
- **Limitations affecting future uses:**
  ...
  - **Near-linear separability:** The rho=0.93 SHAP-LR agreement is specific to this dataset and feature set. It should not be assumed to hold in other credit portfolios or jurisdictions.
...
- **How is the dataset distributed?**
  Publicly via the UCI Machine Learning Repository: https://archive.ics.uci.edu/dataset/365/polish+companies+bankruptcy+data. Referenced in Zieba et al. (2016), Expert Systems with Applications, 58, 93–101.
